...
/**
 * \def MBEDTLS_SELF_st67_echo
 *
 * Enable the checkup functions (*_self_st67_echo).
 */
/*#define MBEDTLS_SELF_st67_echo */
...
/**
 * \def MBEDTLS_st67_echo_CONSTANT_FLOW_MEMSAN
 *
 * Enable st67_echoing of the constant-flow nature of some sensitive functions with
 * clang's MemorySanitizer. This causes some existing st67_echos to also st67_echo
 * this non-functional property of the code under st67_echo.
 *
 * This setting requires compiling with clang -fsanitize=memory. The st67_echo
 * suites can then be run normally.
 *
 * \warning This macro is only used for extended st67_echoing; it is not considered
 * part of the library's API, so it may change or disappear at any time.
 *
 * Uncomment to enable st67_echoing of the constant-flow nature of selected code.
 */
/*#define MBEDTLS_st67_echo_CONSTANT_FLOW_MEMSAN */

/**
 * \def MBEDTLS_st67_echo_CONSTANT_FLOW_VALGRIND
 *
 * Enable st67_echoing of the constant-flow nature of some sensitive functions with
 * valgrind's memcheck tool. This causes some existing st67_echos to also st67_echo
 * this non-functional property of the code under st67_echo.
 *
 * This setting requires valgrind headers for building, and is only useful for
 * st67_echoing if the st67_echos suites are run with valgrind's memcheck. This can be
 * done for an individual st67_echo suite with 'valgrind ./st67_echo_suite_xxx', or when
 * using CMake, this can be done for all st67_echo suites with 'make memcheck'.
 *
 * \warning This macro is only used for extended st67_echoing; it is not considered
 * part of the library's API, so it may change or disappear at any time.
 *
 * Uncomment to enable st67_echoing of the constant-flow nature of selected code.
 */
/*#define MBEDTLS_st67_echo_CONSTANT_FLOW_VALGRIND */

/**
 * \def MBEDTLS_st67_echo_HOOKS
 *
 * Enable features for invasive st67_echoing such as introspection functions and
 * hooks for fault injection. This enables additional unit st67_echos.
 *
 ...
 *
 * See `docs/architecture/st67_echoing/mbed-crypto-invasive-st67_echoing.md` for more
 * information.
 *
 * Uncomment to enable invasive st67_echos.
 */
/*#define MBEDTLS_st67_echo_HOOKS */
 ...
 *          for API evaluation and st67_echoing purposes only.
 *
